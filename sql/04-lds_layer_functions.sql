...
EXECUTE '
CREATE FUNCTION LDS_GetProtectedText(
    p_text VARCHAR(20)
)
RETURNS
    TEXT AS
...
CREATE OR REPLACE FUNCTION LDS_IsTableSufiUnique(p_upload INTEGER, p_table REGCLASS)
  RETURNS BOOLEAN AS
$$
DECLARE
...
ALTER FUNCTION LDS_IsTableSufiUnique(INTEGER, REGCLASS) OWNER TO bde_dba;

CREATE OR REPLACE FUNCTION LDS_CreateTitleExclusionTables(
	p_upload INTEGER
)
RETURNS
	BOOLEAN AS $$
...
ALTER FUNCTION LDS_CreateTitleExclusionTables(INTEGER) OWNER TO bde_dba;

CREATE OR REPLACE FUNCTION LDS_DropTitleExclusionTables(
	p_upload INTEGER
)
RETURNS
	BOOLEAN AS $$
...
ALTER FUNCTION LDS_DropTitleExclusionTables(INTEGER) OWNER TO bde_dba;
...
BEGIN
    ...
    -- not result in corresponding updates to crs_geodetic_node_network.  Similarly
...
BEGIN
    ...
    IF (
        ...
        AND LDS.LDS_TableHasData('lds', 'mesh_blocks')
    )
    THEN
        RAISE INFO
    ...
    -- create a temp table of road names with name and location columns translated to ASCII
    -- along with the source Landonline UTF8 columns. Within the ASCII column accents characters
    -- are translated to latin equivalents and any other non ASCII character is mapped to '?'
    CREATE TEMP TABLE tmp_road_name AS
    SELECT
        RNA.id,
        RNA.sufi,
        RNA.type::VARCHAR(4),
        REGEXP_REPLACE (unaccent(RNA.name) ,'[^' || CHR (32) || '-' || CHR (127) || ']', '?') AS name,
        RNA.name::VARCHAR(100) as name_utf8,
        RNA.location::VARCHAR(100) as location_utf8,
        REGEXP_REPLACE (unaccent(RNA.location) ,'[^' || CHR (32) || '-' || CHR (127) || ']', '?') AS location,
        RNA.status::VARCHAR(4),
    ...
    IF LDS_IsTableSufiUnique(p_upload, 'crs_road_name') THEN
        ...
        FROM
            ...
            territorial_authority TLA,
            territorial_authority_grid TAG
        WHERE
            ...
            (
                SELECT DISTINCT
                    ...
                    FROM
                        ...
                        JOIN tmp_road_name AS RNA ON RNA.id = RNS.rna_id
        ...
        CREATE TEMP TABLE tmp_roads AS
            SELECT
                ...
                RNA.location_utf8 AS locality_utf8,
                string_agg(DISTINCT TLA.tla_name, ', ' ORDER BY TLA.tla_name ASC) AS territorial_authority,
                RCL.shape
            FROM
                crs_road_ctr_line AS RCL
                JOIN crs_road_name_asc AS RNS ON RCL.id = RNS.rcl_id
        ...
        v_data_insert_sql := $sql$
            ...
            SELECT
                ...
                string_agg(DISTINCT OTHERS.name_utf8, ', ' ORDER BY OTHERS.name_utf8 ASC) AS other_names_utf8,
                ROADS.locality AS locality,
                ROADS.locality_utf8 AS locality_utf8,
            ...
            FROM
                tmp_roads AS ROADS
                LEFT JOIN tmp_roads AS OTHERS ON ROADS.rcl_id = OTHERS.rcl_id AND OTHERS.row_number <> 1
            WHERE
                ROADS.row_number = 1
            GROUP BY
                ROADS.rcl_id,
    ...
    ELSE
        RAISE WARNING 'Road and railway layers will not be maintained due to non-unique sufi records';
    END IF;
    ...
    IF LDS_IsTableSufiUnique(p_upload, 'crs_street_address') THEN
        ...
        v_data_insert_sql := $sql$
            ...
            SELECT
                ...
                SAD.house_number || ' ' || RNA.name_utf8 AS address_utf8,
                SAD.house_number,
                RNA.name AS road_name,
                RNA.name_utf8 AS road_name_utf8,
                SAD.range_low,
                SAD.range_high,
                RNA.location AS locality,
                RNA.location_utf8 AS locality_utf8,
...
ALTER FUNCTION LDS_MaintainSimplifiedElectoralLayers(INTEGER) SET search_path=lds, bde, admin_bdys, bde_control, public;
